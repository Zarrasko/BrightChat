 ...
 * [Notifications.post] raises a moment earlier — so with both apps switched on, a message was one
 * buzz and **two boxes**, one landing on top of the other.
 *
 * One of them has to stand down and it should be this one. BrightControl knows about every app on
 * the phone; this app knows about one, and the box it draws is the box BrightControl would have
 * drawn anyway.
 *
 * ### What does not change
 *
 * The buzz and the shade notification. Both happen before the gate in [HeadsUp.show], and both must
 * keep happening: the notification is the record BrightControl reads and LightGlance's dot counts,
 * and if this app went quiet as well as boxless, a phone where BrightControl's own listener grant
 * had lapsed would say nothing at all about a text.
 */
object AlertOwner {
     ...
     * BrightControl gives this app its box back on the next message, with no setting to find.
     */
    fun ownedElsewhere(context: Context): Boolean {
        if (!Store.alertsOwnedElsewhere(context)) return false
        return runCatching {
 ...
 * message arrives on — the one moment this app exists for. The cost of the other direction is
 * staleness, which BrightControl pays for by sending it often and unprompted: on its every launch,
 * the moment its listener grant lands, and at boot. So a missed broadcast corrects itself rather
 * than sticking.
 *
...
class AlertOwnerReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        Store.setAlertsOwnedElsewhere(context, intent.getBooleanExtra(EXTRA_OWNED, false))
    }
