package com.gios.lightchat.report
...
data class Failure(
    /** In the app's own words, lower case, completing "LightChat could not …". */
    val what: String,
 ...
 * ordinary never arrive at all. So the places that already catch an error and put a sentence on
 * the screen hand it here as well, and the app asks.
 *
 * **The nagging is the thing to get right.** A calendar that cannot be reached fails on every
 * hourly sync, and an app that asks to report it twelve times before lunch gets its reporting
 * turned off. So the same failure asks once an hour at most, and only ever while you are looking
 * at the app — a failure noticed by a background alarm waits, and if it is still broken when you
 * next open the app it asks then.
 */
object Trouble {

    /** Long enough that a failing hourly sync asks once, not once an hour. */
