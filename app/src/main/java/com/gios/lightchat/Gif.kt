 ...
 * ad slots and analytics ids; what a phone this size needs is a small one to draw in a grid, a
 * bigger one to send, and enough identity to recognise the same GIF again when it turns up in
 * somebody's favourites. Everything else is dropped at the parse boundary
 * ([com.gios.lightchat.api.KlipyApi.Companion.parsePage]) so no other part of the app has to know
 ...
 *
 * [sendUrl] and [previewUrl] are both `.gif` URLs, never mp4 or webp: the file is going to
 * Messages on a Mac and then to whatever the recipient reads iMessage on, and a GIF is the one
 * animated format all of them draw inline.
 */
data class Gif(
    val id: String,
    val title: String,
 ...
 *
 * JSON rather than the newline-joined form [com.gios.lightchat.api.Store.favorites] uses for chat
 * guids, and for the same reason [NewsletterJson] is JSON: a GIF carries a provider-written
 * *title*, and a delimiter trick that is safe for guids is not safe for a string somebody else
 * chose. Free of Android so it has a test rather than a phone.
 *
