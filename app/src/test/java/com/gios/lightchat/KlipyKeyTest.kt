...
import com.gios.lightchat.api.KlipyKey
import java.io.File
...
class KlipyKeyTest {
    ...
    fun `pad matches the build script`() {
        ...
        val script = sequenceOf(File("build.gradle.kts"), File("app/build.gradle.kts"))
            .firstOrNull { it.isFile }
        assertNotNull("app/build.gradle.kts not found from the test's working directory", script)
        assertTrue(
            "the pad in the build script's scramble() no longer matches KlipyKey.PAD",
            script!!.readText().contains("""val pad = "${KlipyKey.PAD}".toByteArray(Charsets.UTF_8)"""),
        )
    }
}
